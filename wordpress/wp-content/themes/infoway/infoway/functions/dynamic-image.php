 ...
 * @return mixed|string
 */
function infoway_document_root($path) {
 ...
 */
function infoway_image_resize($img_url, $width, $height, $crop = true, $quality = 100) {
    ...
    if ($_SERVER['HTTP_HOST'] != $host && $host != '') {  // The image is not locally hosted
        ...
        $file_path = infoway_document_root($file_path['path']) . $file_path['path'];
    }
    ...
    if ($source[1] > $width || $source[2] > $height) {
        ...
        $new_img_path = wp_get_image_editor($file_path, $width, $height, $crop, $suffix, $upload_path, $quality);
 ...
 * @return type 
 */
function infoway_thumbnail_resize($attach_id = null, $img_url = null, $width, $height, $crop = false, $jpeg_quality = 90) {
    ...
    if ($image_src[1] > $width || $image_src[2] > $height) {
        ...
        $new_img_path = wp_get_image_editor($file_path, $width, $height, $crop, $jpeg_quality);
        $new_img_size = getimagesize($new_img_path);
