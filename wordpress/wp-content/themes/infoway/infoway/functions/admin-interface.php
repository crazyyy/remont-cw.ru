...
function infoway_optionsframework_add_admin() {
    global $query_string;
    
    $themename =  "InfoWay";      
    $shortname =  infoway_get_option('of_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  infoway_get_option('of_template'); 
			infoway_reset_options($options,'optionsframework');
			header("Location: admin.php?page=optionsframework&reset=true");
			die;
		}
    }  
$of_page = add_theme_page($themename,'Theme Options','edit_theme_options', 'optionsframework', 'infoway_optionsframework_options_page', 'div');	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'infoway_load_only');
} 
add_action('admin_menu', 'infoway_optionsframework_add_admin');
/*-----------------------------------------------------------------------------------*/
/* Options Framework Reset Function - of_reset_options */
/*-----------------------------------------------------------------------------------*/
function infoway_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;	
	...
	if($page == 'optionsframework'){
		infoway_delete_option( 'of_options' );
	}
}
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function infoway_optionsframework_options_page(){
    $options =  infoway_get_option('of_template');      
    $themename =  infoway_get_option('of_themename');
?>
<div class="trail-notify">
        <h1>Get InfoWay Theme PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are Using the Lite Version of InfoWay Theme. Upgrade to Pro for extra features like Lead Capture Integration, Home Page Slider, Contact Page, Gallery Page, FullWidth Page Templates, Multiple Color Options and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/infoway-wordpress-theme-with-built-in-lead-capture/" target="blank">Upgrade to Infoway PRO Theme here >></a>
    </div>
<div class="wrap" id="of_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <?php 
        ...
        $return = infoway_optionsframework_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="По умолчанию" class="button submit-button reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
...
<script type="text/javascript">
			...
			new AjaxUpload(clickedID, {
				  ...
				  onComplete: function(file, response) {
					...
					clickedObject.text('Загрузить изображение');	
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - infoway_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'infoway_ajax_callback');
function infoway_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				infoway_update_option( $clickedID , $uploaded_file['url'] );
	...
	elseif($save_type == 'image_reset'){
			...
			infoway_delete_option( $id );
	
	}	
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		$data = $_POST['data'];		
		parse_str($data,$output);
		//verify the _wpnonce field
		$nonce=(isset($output['_wpnonce']))?$output['_wpnonce']:'';
		if (! wp_verify_nonce($nonce,'theme-update-option') ){
		...
		foreach($options as $option_array){
			$id = $option_array['id'];
			$old_value = infoway_get_option($id);
			$new_value = '';
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								infoway_update_option( $id, stripslashes($new_value));
							}
						}                 
					}
					elseif($new_value == '' && $type == 'checkbox'){ // Checkbox Save
						
						infoway_update_option($id,'false');
					}
					elseif ($new_value == 'true' && $type == 'checkbox'){ // Checkbox Save
						
						infoway_update_option($id,'true');
					}
					elseif($type == 'multicheck'){ // Multi Check Save
						...
						foreach ($option_options as $options_id => $options_value){
							...
							if(!isset($output[$multicheck_id])){
							  infoway_update_option($multicheck_id,'false');
							}
							else{
							   infoway_update_option($multicheck_id,'true'); 
							}
						}
					} 
					elseif($type == 'typography'){
						...
						infoway_update_option($id,$typography_array);
					...
					elseif($type == 'border'){
						...
						infoway_update_option($id,$border_array);
...
/*-----------------------------------------------------------------------------------*/
function infoway_optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			$val = $value['std'];
			$std = infoway_get_option($value['id']);
			if ( $std != "") { $val = $std; }
		...
		break;
			...
			$select_value = infoway_get_option($value['id']);
		...
		case "upload":
			$value['std'] = '';
			if (isset($value['std'])){
			$output .= infoway_optionsframework_uploader_function($value['id'],$value['std'],null);
			}
		break;
		case "upload_min":
			
			$output .= infoway_optionsframework_uploader_function($value['id'],$value['std'],'min');
			
		break;
		case "color":
			$val = $value['std'];
			$stored  = infoway_get_option( $value['id'] );
		...
		case "typography":
			...
			$default = $value['std'];
			$typography_stored = infoway_get_option($value['id']);
		...
		case "border":
			...
			$default = $value['std'];
			$border_stored = infoway_get_option( $value['id'] );
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					$id = $array['id']; 
					$std = $array['std'];
					$saved_std = infoway_get_option($id);
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - infoway_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
function infoway_optionsframework_uploader_function($id,$std,$mod){
	...
	$uploader = '';
    $upload = infoway_get_option($id);
	
	if($mod != 'min') { 
			$val = $std;
            if ( infoway_get_option( $id ) != "") { $val = infoway_get_option($id); }
            $uploader .= '<input class=\'of-input\' name=\''. $id .'\' id=\''. $id .'_upload\' type=\'text\' value=\''. str_replace( "'" , ""  , $val ) .'\' />';
	}
	
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">Загрузить изображение</span>';
	
	if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
	
	$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">Удалить</span>';
	$uploader .='</div>' . "\n";
